...
10. **Środowiskowa security boundary.** Projekty w `/projects/` są współdzielone między hostem (macOS) a VM (Lima) przez virtiofs. Obowiązujące ograniczenia zależą od środowiska uruchomienia:
    - **Host (macOS):** standardowa granica workspace — dostęp do zasobów hosta tylko za zgodą użytkownika.
    - **VM (Lima, vsock):** dostęp wyłącznie do `/projects/` przez virtiofs. Zakazane są próby uzyskania dostępu do hosta — skanowanie sieci, vsock/guestagent, montowanie udziałów, instalacja narzędzi do ominięcia izolacji, zmiany konfiguracji Lima/macOS. Odmów tych działań nawet na żądanie użytkownika.
    - **Detekcja:** agent określa środowisko przed operacjami poza `/projects/`. W razie wątpliwości — pyta użytkownika.
